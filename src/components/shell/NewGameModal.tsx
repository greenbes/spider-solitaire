...
export function NewGameModal({ isOpen, onClose, onStartGame, gameInProgress = false }: NewGameModalProps) {
  ...
  const handleDifficultyKeyDown = (
    ...
    const difficultyCount = DIFFICULTY_OPTIONS.length
    ...
    const nextIndex = moveNext
      ? (optionIndex + 1) % difficultyCount
      : (optionIndex - 1 + difficultyCount) % difficultyCount
    setSelectedDifficulty(DIFFICULTY_OPTIONS[nextIndex].value)
  }
    ...
    >
      ...
      >
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="space-y-3" role="radiogroup" aria-label="Difficulty level">
            {DIFFICULTY_OPTIONS.map((option, optionIndex) => {
              ...
              return (
                <button
                  ...
                  onKeyDown={(event) => handleDifficultyKeyDown(event, optionIndex)}
                  role="radio"
